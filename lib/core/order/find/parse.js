...
module.exports = function(query = {}) {
    ...
    if(query.user) { $inFn("user"); }
    if(query.brands) { mquery.brands = {$all: query.brands}; }
    if(query.companies) { mquery.companies = {$all: query.companies}; }
    if(query.status) { $inFn("status"); }
    if(query.listSubscription) { $inFn("listSubscription"); }
    if(query.hero) { $inFn("hero"); }
