...
module.exports = function(query = {}) {
    ...
    if(lodash.isString(query.hash)) {
        $regxFn("hash");
    } else if(lodash.isObject(query.hash) && lodash.size(query.hash) > 0) {
        mquery.hash = {};

        lodash.forOwn(query.hash, (val,key) => {
            mquery.hash[`$${key}`] = val;
        });
    }
    ...
    if(lodash.isNumber(query.stock)) {
        mquery.stock = query.stock;
    }
    else if(lodash.isObject(query.stock) && lodash.size(query.stock) > 0) {
        mquery.stock = {};

        lodash.forOwn(query.stock, (val,key) => {
            mquery.stock[`$${key}`] = parseInt(val);
        });
    }
