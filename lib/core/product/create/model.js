...
module.exports.Schema = Joi.object().keys({
    ...
    costValue: Joi.number().integer().default(0),
    ...
    tags: Joi.array().items(Joi.string()).empty(null).default([]),
    ...
    // Hash is composed by category and nameId.
    hash: Joi.string(),
    ...
    supplyType: Joi.string().only([
        Types.SupplyType.ON_DEMAND,
        Types.SupplyType.ON_STOCK
    ]).default(Types.SupplyType.ON_DEMAND),

    stock: Joi.number().integer().empty(null).default(0),
    stockValue: Joi.number().integer().empty(null).default(0),
