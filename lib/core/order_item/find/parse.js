...
module.exports = class Parser {
    ...
    static parseQuery(query = {}) {
        ...
        if(query.product) { $inFn("product"); }
        if(query.order) { $inFn("order"); }
        if(query.brand) { $inFn("brand"); }
        if(query.company) { $inFn("company"); }
        if(query.status) { $inFn("status"); }

        if(query.pickupDate && lodash.size(query.pickupDate) > 0) {
            mquery.pickupDate = {};

            lodash.forOwn(query.pickupDate, (val,key) => {
                mquery.pickupDate[`$${key}`] = val;
            });
        }

        if(query.deliverDate && lodash.size(query.deliverDate) > 0) {
            mquery.deliverDate = {};

            lodash.forOwn(query.deliverDate, (val,key) => {
                mquery.deliverDate[`$${key}`] = val;
            });
        }
