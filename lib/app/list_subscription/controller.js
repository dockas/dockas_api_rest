...
module.exports = class Ctrl {
    ...
    constructor() {
        ...
        // Log
        logger.info("cron pattern", {runWeekdays,cronPattern})
    ...
    async handleCronJob({todayDate=null}={}) {
        let result,
            ...
            todayMoment = todayDate?moment(todayDate):moment(),
        ...
        for(let subscription of subscriptions) {
                ...
                items.push(
                    OrderItemSrv.client.create(
                        new OrderItemSrv.types.Data({
                            ...
                            brand: lodash.get(item,"product.brand"),
                            order: orderId,
                            quantity: item.quantity,
                            priceValue: lodash.get(item, "product.priceValue"),
                            costValue: lodash.get(item, "product.costValue"),
                            status: lodash.get(item, "product.supplyType") == ProductSrv.types.SupplyType.ON_STOCK ?
                                OrderItemSrv.types.Status.STOCKED :
                                OrderItemSrv.types.Status.PENDING,
                            stockedQuantity: lodash.get(item, "product.supplyType") == ProductSrv.types.SupplyType.ON_STOCK ?
                                item.quantity : 0,
                            deliverDate: order.deliverDate,
                            pickupDate: lodash.get(item, "product.supplyType") == ProductSrv.types.SupplyType.ON_DEMAND ? 
                                moment(order.deliverDate).subtract(config.order.pickupDaysBeforeDeliver, "days").toISOString() :
                                undefined
                        })
                    )
                );
            ...
            // Await order items to be all created
            try {
                items = await Promise.all(items);
                logger.info("order items create success", {count: items.length});
            }
            catch(error) {
                logger.error("order items create error", error);
            }

            // Set items id's to order
            order.items = lodash.map(items, "_id");
        ...
        for(result of results) {
            let {order,items,subscription} = result;
