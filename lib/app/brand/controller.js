...
module.exports = class Ctrl {
    ...
    constructor() {
        Kafka.on("order:item_status:updated", this.handleOrderItemStatusUpdatedEvent.bind(this));
    }
     ...
     */
    async handleOrderItemStatusUpdatedEvent(message={}) {
        let transfer,
            logger = Logger.create("handleOrderItemStatusUpdatedEvent", message.trackId);
        ...
        try {
            let results = await populate([item], ["product", "product.brand"], {
        ...
        if(item.product.supplyType != "on_demand" || item.status != "stocked") {return;}

        // Let's generate a transfer for the brand wallet 
        // with total value of the item.
        let totalPrice = item.quantity * item.priceValue;

        // Try to create the transfer
        try {
            transfer = await TransferSrv.client.create(
                new TransferSrv.types.Data({
                    wallet: lodash.get(item,"product.brand.wallet"),
                    value: totalPrice,
                    fees: [{
                        type: TransferSrv.types.FeeType.PERCENTUAL,
                        value: config.fee.onDemand
                    }]
                }),
                logger.trackId
            );
        ...
        for(let owner of lodash.get(item, "product.brand.owners")||[]) {
