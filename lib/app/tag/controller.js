...
module.exports = class Ctrl {
    ...
    /**
     * This function creates a category edge.
     */
    static async createEdge(req, res) {
        let result,
            data = req.body,
            logger = Logger.create("createEdge", req.trackId);

        logger.info("enter", data);

        // Try to create a new entity.
        try {
            result = await TagSrv.client.createCategoryEdge(
                new TagSrv.types.CategoryEdgeData(data),
                logger.trackId
            );
        }
        catch(error) {
