...
module.exports = class Ctrl {
    ...
    static async create(req, res) {
        ...
        if(data.users.indexOf(req.uid) < 0) {
            data.users.push(req.uid);
        }
    ...
    static async find(req, res) {
        let records,
            ...
            query = new ListSrv.types.Query(req.query);

        logger.info("enter", {query: req.query});

        // Try to find records
        try {
            records = await ListSrv.client.find(query, logger.trackId);
        }
        catch(error) {
    ...
    static async update(req, res) {
        ...
        try {
            ...
            if(result.users.indexOf(req.uid) < 0) {
